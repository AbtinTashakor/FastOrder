...
use app::{
    models::cart::{Cart, CartItemView, CartStatus, CartView},
    repos::cart::CartRepo,
};
...
impl PgCartRepo {
    ...
    fn map_cart(row: CartRow) -> Cart {
        Cart {
            id: row.id,
            user_id: row.user_id,
            status: match row.status.as_str() {
                "active" => CartStatus::Active,
                "confirming" => CartStatus::Confirming,
                _ => CartStatus::Locked,
            },
        }
    }

    async fn load_cart_items(&self, cart_id: Uuid) -> Result<CartView> {
...
impl CartRepo for PgCartRepo {
    ...
    async fn find_confirming_cart(&self, user_id: Uuid) -> Result<Option<Cart>> {
        let row = sqlx::query_as::<_, CartRow>(
            ...
            SELECT id, user_id, status
            FROM carts
            WHERE user_id = $1 AND status = 'confirming'
    ...
    async fn get_cart_view(&self, cart_id: Uuid) -> Result<CartView> {
        self.load_cart_items(cart_id).await
    }
